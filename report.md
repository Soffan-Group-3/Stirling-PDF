...
##### The architecture and purpose of the system

###### Introduction
Stirling-PDF is designed as a locally hosted web application for performing a variety of operations on PDFs. The primary purpose of the system is to allow users to manipulate PDF files safely and efficiently . By running locally (via Docker), all files are either processed client-side or exist temporarily on the server during task execution. This design ensures that no sensitive PDF data persists on the server.

###### System purpose

The application supports over 50 PDF operations. These include but are not limited to splitting, merging, rotating, converting, annotating, OCR processing, and more. Some other capabilities are:


- Document Manipulation: Reordering, cropping, and scaling pages.
- Conversion Tasks: Converting PDFs to/from images, Word documents, HTML, and other formats.
- Security Operations: Adding/removing passwords, setting permissions, watermarking, and sanitizing documents.
- Advanced Processing: Automating multi-step operations through custom pipelines


###### Architectural Overview
Stirling-PDF is built as a web application with a clear separation between its backend services and the user interface. The backend is developed using Spring Boot, while Thymeleaf is used to render the HTML pages served to users. This setup keeps the front-end and back-end semi-separate. The API handles PDF processing, and Thymeleaf delivers the web interface.

The server is ran and maintained through Springboot. Springboot does multiple things, and makes a lot of work with the server easier. Most importantly Springboot exposes a set of REST ``endpoints``, in which the front-end can make requests. In the project each endpoint corresponds to a specific PDF operation. These endpoints are connected to `Controllers`, in which a `request` is sent when the endpoint gets a HTML request. The `controller` then uses the information in the `request` to delegate tasks to a `service`(s), before ultimately sending a response back. In this design each function is isolated to its own controller and service layer. Which could theoretically make it easier to extend and maintain.

The front-end of the application is made with Thymeleaf, and more specifically Thymeleaf templates. These are HTML templates that when used are populated with the needed information form the back-end, like names and titles. 

There are however, some cases where the back-end actually does nothing and all of the operations are done on the front end. In these cases then `Javascript` is used with the dependency/library `pdf-lib` to make direct changes to the pdfs without relying on the back-end. This is often done with operations that require a lot of user input, like changing `saturation` of the document. 

A brief and simplified overview of the architecture may be seen in the following diagram:

![architecture overview](images/StirlingPDFWithDocker.jpg)

###### Dataflow

The general dataflow of Stirling pdf may be described by the following:

1.  A user uploads a PDF through a form generated by Thymeleaf.
2. The file and any associated options are sent to the Spring Boot API.
3. The API identifies the desired PDF operation and calls the relevant service.
4. The service processes the file.
5. The processed file is returned to the user for download.
6. Temporary data is cleaned
